...
from yolo_v11s_interfaces.msg import Detection2D
from geometry_msgs.msg import Pose2D

# Define and parse user input arguments
parser = argparse.ArgumentParser()
parser.add_argument('--model', help='Path to YOLO model file (example: "runs/detect/train/weights/best.pt")',
                    default='tray_detection_model/tray_detection_model.pt')
...
# Initialize ROS2
rclpy.init()
ros_node = YoloTrayDetectionNode()
...
def apply_custom_nms(detections, nms_threshold, confidence_threshold):
    ...
    for i in range(len(detections)):
        ...
        if conf > confidence_threshold:
            ...
            boxes.append(xyxy)
            scores.append(conf)
            class_ids.append(int(detections[i].cls.item()))
...
class YoloTrayDetectionNode(Node):
    def __init__(self):
        super().__init__('yolo_tray_detection_node')
        self.publisher_ = self.create_publisher(Detection2D, 'yolo_detections', 10)
        self.get_logger().info('YOLO Tray Detection Node initialized')

    def publish_detections(self, detections, valid_indices, labels, aligned_depth_frame):
        """Publish detection results to ROS2 topic using custom Detection2D message"""
        ...
        for idx in valid_indices:
            ...
            # Create custom Detection2D message
            detection_msg = Detection2D()
            detection_msg.class_name = classname
            detection_msg.score = float(conf)
            detection_msg.x_center = float((xmin + xmax) / 2)
            detection_msg.y_center = float((ymin + ymax) / 2)
            detection_msg.width = float(xmax - xmin)
            detection_msg.height = float(ymax - ymin)

            # Publish individual detection
            self.publisher_.publish(detection_msg)

        self.get_logger().debug(f'Published {len(valid_indices)} detections')
...
while True:
    ...
    # Run YOLO inference on the color image with NMS parameters
    results = model(
        color_image, 
        conf=min_thresh,
        iou=nms_thresh,
    ...
    # Apply additional custom NMS for better filtering
    valid_indices = apply_custom_nms(detections, nms_thresh, min_thresh)
    ...
    for idx in valid_indices:
        i = int(idx)  # Convert to int if needed
    ...
    ros_node.publish_detections(detections, valid_indices, labels, aligned_depth_frame)
    ...
    # Display NMS info
    cv2.putText(display_image, f'NMS: {nms_thresh:.2f} | Thresh: {min_thresh:.2f}', (10, 80), 
